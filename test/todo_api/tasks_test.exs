defmodule TodoApi.TasksTest do
  ...
  describe "todos" do
    ...
    test "create_todo/1 with valid data creates a todo" do
      valid_attrs = %{description: "some description", title: "some title", deadline: ~D[2023-09-23], completed: true}

      assert {:ok, %Todo{} = todo} = Tasks.create_todo(valid_attrs)
      assert todo.description == "some description"
      assert todo.title == "some title"
      assert todo.deadline == ~D[2023-09-23]
    ...
    test "update_todo/2 with valid data updates the todo" do
      todo = todo_fixture()
      update_attrs = %{description: "some updated description", title: "some updated title", deadline: ~D[2023-09-24], completed: false}

      assert {:ok, %Todo{} = todo} = Tasks.update_todo(todo, update_attrs)
      assert todo.description == "some updated description"
      assert todo.title == "some updated title"
      assert todo.deadline == ~D[2023-09-24]
